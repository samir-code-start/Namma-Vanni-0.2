...
def _sarvam_tts(text: str, lang: str) -> bytes | None:
    """Call Sarvam TTS API (bulbul:v3) for Kannada ONLY. Returns audio bytes or None.

    For Hindi and English, this returns None immediately so generate_tts()
    falls through to Edge TTS, which has better hi/en support.
    """
    # Sarvam TTS is used ONLY for Kannada
...
def generate_tts(text: str, lang: str) -> str:
    """Synthesise verification prompt.

    Routing:
      - Kannada (kn): Sarvam TTS bulbul:v3 (primary) → Edge TTS (fallback)
      - Hindi (hi) / English (en): Edge TTS directly (Sarvam TTS skipped)
    """
    if MOCK_MODE:
    ...
    # Try Sarvam TTS only for Kannada (returns None immediately for hi/en)
    ...
    # Edge TTS for hi/en (and Kannada fallback if Sarvam TTS failed)
    logger.info("[EDGE TTS] Using Edge TTS for lang=%s...", lang)
